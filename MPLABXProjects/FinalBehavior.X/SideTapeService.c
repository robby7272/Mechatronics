...
uint8_t InitSideTapeService(uint8_t Priority)
{
    ES_Event ThisEvent;
    PORTW05_TRIS = 1;
    PORTW07_TRIS = 1;
    ES_Timer_InitTimer(SideTapeTimer, TAPE_DEBOUNCE);
 ...
 *        Returns TRUE if successful, FALSE otherwise
 * @author J. Edward Carryer, 2011.10.23 19:25 */
uint8_t PostSideTapeService(ES_Event ThisEvent)
{
 ...
 * @author J. Edward Carryer, 2011.10.23 19:25 */
ES_Event RunSideTapeService(ES_Event ThisEvent)
{
    ...
    uint8_t tapevalue = PORTW05_BIT;// read the tape
...
#else
            PostSideTapeService(ReturnEvent);
#endif   
        }
        break;
