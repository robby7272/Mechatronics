...
uint8_t InitReverseTowerShark(void)
{
    ES_Event returnEvent;

    CurrentState = InitPSubState;
    returnEvent = RunReverseTowerShark(INIT_EVENT);
...
ES_Event RunReverseTowerShark(ES_Event ThisEvent)
{
...
//        break;
    ...
    case Advance: // in the first state, replace this with correct names
        ...
        case UNBUMPED:
            ...
            ThisEvent.EventType = ES_NO_EVENT;
            break;
        case REVERSE_SIDE_BLACK:
            if(PORTW05_BIT == 0 && AD_ReadADPin(AD_PORTV3) > 530 && AD_ReadADPin(AD_PORTV4) > 530){
            ...
            break;
        case SIDE_WHITE:
            if(PORTW07_BIT == 1 && AD_ReadADPin(AD_PORTV3) > 530 && AD_ReadADPin(AD_PORTV4) > 530){
                ES_Timer_StopTimer(11);
                ES_Timer_InitTimer(3, 500);
                nextState = Scoot;
                makeTransition = TRUE;
    ...
    case Scoot: // in the first state, replace this with correct names
        //Move_Sideways(400, LEFT);
        //MOTOR_3(550, REVERSE);
        Motors_Stop();
        switch (ThisEvent.EventType) {
        case ES_TIMEOUT:
    ...
    if (makeTransition == TRUE) { // making a state transition, send EXIT and ENTRY
        ...
        RunReverseTowerShark(EXIT_EVENT); // <- rename to your own Run function
        CurrentState = nextState;
        RunReverseTowerShark(ENTRY_EVENT); // <- rename to your own Run function
    }
