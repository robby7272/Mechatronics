...
uint8_t InitTapeService(uint8_t Priority)
{
    ...
    PORTW04_TRIS = 1;
    PORTW06_TRIS = 1;
    ES_Timer_InitTimer(TapeTimer, TAPE_DEBOUNCE);
...
ES_Event RunTapeService(ES_Event ThisEvent)
{
    ...
    unsigned char tapevalue = ((PORTW04_BIT << 1) | (PORTW06_BIT << 2)) & (0b0110); // read the tape
    ...
    case ES_TIMEOUT:
        ...
        if (tapevalue != 0) { // is tape?
            curEvent = FLOOR_BLACK;
        } else {
            curEvent = FLOOR_WHITE;
        }
