...
static DeadBotState_t CurrentState = InitPSubState; // <- change name to match ENUM
...
/**
 * @Function InitDeadBot(uint8_t Priority)
...
uint8_t InitDeadBot(void)
{
    ES_Event returnEvent;

    CurrentState = InitPSubState;
    returnEvent = RunDeadBot(INIT_EVENT);
...
/**
 * @Function RunDeadBot(ES_Event ThisEvent)
...
ES_Event RunDeadBot(ES_Event ThisEvent)
{
    uint8_t makeTransition = FALSE; // use to flag transition
    DeadBotState_t nextState; // <- change type to correct enum
    ...
    if (makeTransition == TRUE) { // making a state transition, send EXIT and ENTRY
        ...
        RunDeadBot(EXIT_EVENT); // <- rename to your own Run function
        CurrentState = nextState;
        RunDeadBot(ENTRY_EVENT); // <- rename to your own Run function
    }
