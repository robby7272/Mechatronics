...
uint8_t InitBumperService(uint8_t Priority)
{
    ...
    PORTV07_TRIS = 1;
    PORTV08_TRIS = 1;
    ES_Timer_InitTimer(BumperTimer, BUMPER_DEBOUNCE);
...
ES_Event RunBumperService(ES_Event ThisEvent)
{
    ...
    unsigned char bumpervalue = ((PORTV06_BIT) | (PORTV07_BIT << 1) | (PORTV08_BIT << 2)) & (0b0111); // read the bump
    uint8_t sidehoe = PORTV06_BIT;
    ...
    case ES_TIMEOUT:
        ES_Timer_InitTimer(BumperTimer, BUMPER_DEBOUNCE);
        if (bumpervalue != 0) { // is bump?
            if(sidehoe != 0){
                curEvent = SIDE_BUMPED;
            } else {
                curEvent = BUMPED;
            }
        }
